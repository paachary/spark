...
class StreamStatusCount(spark: SparkSession, logFileLocation : String ) {
  ...
  val streamingDF: DataFrame = accessLogStreamingDF.
    ...
    groupBy($"STATUS_CODE", window($"REQUESTED_DATE_TIME", "1 day")).
    agg(count("STATUS_CODE") as "STATUS_COUNT")

  streamingDF.
    writeStream.
    format("console").
    queryName("status_per_minute").
    outputMode("complete").
    start().
    awaitTermination()
